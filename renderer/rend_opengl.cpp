...
#include <GL/gl.h>
#endif
#include "glload.h"	//contains the OpenGL DLL binding code
...
//#include "module.h"
#undef GL_PROJECTION
#define GL_PROJECTION LGL_PROJECTION
#undef GL_MODELVIEW
#define GL_MODELVIEW LGL_MODELVIEW
...
static int OpenGL_active_tex=0;
static bool OpenGL_tex1_enabled=false;
...
void rGL_SetAlphaType (sbyte atype)
{
	...
	if (OpenGL_multitexture && OpenGL_active_tex) {
		dglActiveTextureARB(GL_TEXTURE0);
		OpenGL_active_tex = 0;
	}
...
void rGL_FillRect (ddgr_color color,int x1,int y1,int x2,int y2)
{
	...
	dglScissor (x1+OpenGL_state.clip_x1,y1+OpenGL_state.clip_y1,x2+OpenGL_state.clip_x1,y2+OpenGL_state.clip_y1);
	dglClearColor ((float)r/255.0f,(float)g/255.0f,(float)b/255.0f,0);
	dglClear (GL_COLOR_BUFFER_BIT|GL_DEPTH_BUFFER_BIT);
	dglScissor (OpenGL_state.clip_x1,OpenGL_state.clip_y1,OpenGL_state.clip_x2,OpenGL_state.clip_y2);
...
void rGL_DrawFlatPolygon(g3Point **p,int nv) {
	if (OpenGL_multitexture)
		rGL_SetMultitextureBlendMode(false);
	ddgr_color color = OpenGL_state.cur_color;
	...
	for (int i = 0; i < nv; i++) {
		...
		if (OpenGL_state.cur_alpha_type & ATF_VERTEX)
			alpha = pnt->p3_uvl.a * Alpha_multiplier; // * OpenGL_Alpha_factor;
		if (OpenGL_state.cur_light_state == LS_NONE)
			dglColor4f((float)GR_COLOR_RED(color)/255.0f,
				(float)GR_COLOR_GREEN(color)/255.0f,
				(float)GR_COLOR_BLUE(color)/255.0f, alpha);
		else if (OpenGL_state.cur_color_model == CM_MONO)
			dglColor4f(pnt->p3_uvl.l, pnt->p3_uvl.l, pnt->p3_uvl.l,alpha);
		else
			dglColor4f(pnt->p3_uvl.r,pnt->p3_uvl.g,pnt->p3_uvl.b,alpha);
		float z = 1.0f - 1.0f / (pnt->p3_vec.z + Z_bias);
		z = max(min(z, 1.0f), 0.0f);
		dglVertex3f((float)x_add + pnt->p3_sx,(float)y_add + pnt->p3_sy,-z);
	}
	dglEnd();
	//OpenGL_verts_processed = OpenGL_verts_processed + param_2;
	//OpenGL_polys_drawn = OpenGL_polys_drawn + 1;
}
...
void rGL_DrawPolygon (int handle,g3Point **p,int nv,int map_type)
{
	...
	{
		rGL_DrawFlatPolygon (p,nv);
		return;
	}
	if (Overlay_type && OpenGL_multitexture) {
		rGL_DrawMultitexturePolygon(handle,p,nv,map_type);
		return;
	}
	...
	for (i=0;i<nv;i++,vertp++,texp++,colorp++)
	{
		...
		if (OpenGL_state.cur_light_state==LS_NONE)
		{
			...
			colorp->b=1;
			colorp->a=alpha;
		} else if (OpenGL_state.cur_light_state==LS_FLAT_GOURAUD) {
			colorp->r=fr;
			colorp->g=fg;
			colorp->b=fb;
			colorp->a=alpha;
...
// ======================
//
// Takes nv vertices and draws the polygon defined by those vertices.  Uses lightmap "handle"
// as a texture
void rGL_DrawMultitexturePolygon (int handle,g3Point **p,int nv,int map_type)
{
	g3Point *pnt;
	int i;
	float alpha;
	vector *vertp;
	color_array *colorp;
	tex_array *texp, *tex2p;
	int w, h;
	
	ASSERT (nv<100);

	w = GameLightmaps[Overlay_map].width;
	h = GameLightmaps[Overlay_map].height;
	float scale =  1.0f / GameLightmaps[Overlay_map].square_res;

	if (!nv)
		return;

	float x_add=OpenGL_state.clip_x1;
	float y_add=OpenGL_state.clip_y1;

	alpha=Alpha_multiplier;

	vertp=&GL_verts[0];
	texp=&GL_tex_coords[0];
	tex2p=&GL_tex_coords2[0];
	colorp=&GL_colors[0];

	// Specify our coordinates
	for (i=0;i<nv;i++,vertp++,texp++,tex2p++,colorp++)
	{
		pnt=p[i];
		...
		tex2p->s=pnt->p3_u2*texw*w*scale;
		tex2p->t=pnt->p3_v2*texw*h*scale;
		tex2p->r=0;
		tex2p->w=texw;
	...
	rGL_MakeWrapTypeCurrent (handle,map_type,0);
	rGL_MakeFilterTypeCurrent(handle,map_type,0);
	rGL_MakeBitmapCurrent (Overlay_map,1,1);
	rGL_MakeWrapTypeCurrent (Overlay_map,1,1);
	rGL_MakeFilterTypeCurrent(Overlay_map,1,1);
	rGL_SetMultitextureBlendMode(true);
...
// end
	...
	{
		opengl_packed_Upload_data=(ushort *)mem_malloc (640*480*2);
	...
	{
		opengl_Upload_data=(uint *)mem_malloc (640*480*4);
...
void rGL_TranslateBitmapToOpenGL (int texnum,int bm_handle,int map_type,int replace,int tn)
{
	...
	if (OpenGL_multitexture && OpenGL_active_tex != tn) {
		dglActiveTextureARB(GL_TEXTURE0 + tn);
		OpenGL_active_tex = tn;
	}
...
int rGL_InitCache ()
{
	...
	if (OpenGL_multitexture) {
		dglActiveTextureARB (GL_TEXTURE1);
		dglTexEnvf (GL_TEXTURE_ENV,GL_TEXTURE_ENV_MODE,GL_MODULATE);
		dglActiveTextureARB (GL_TEXTURE0);
	}
...
void rGL_SetDefaults()
{
	...
	OpenGL_active_tex=9999999;
	OpenGL_tex1_enabled=false;
	...
	dglHint (GL_PERSPECTIVE_CORRECTION_HINT,GL_FASTEST);
	dglHint (GL_FOG_HINT,GL_FASTEST);

  if (OpenGL_multitexture != 0) {
    dglActiveTextureARB(GL_TEXTURE1);
    dglClientActiveTextureARB(GL_TEXTURE1);
    dglEnableClientState(GL_TEXTURE_COORD_ARRAY);
    dglTexCoordPointer(4,GL_FLOAT,0,GL_tex_coords2);
    dglHint(GL_PERSPECTIVE_CORRECTION_HINT,GL_NICEST);
    dglHint(GL_FOG_HINT,GL_FASTEST);
    dglClientActiveTextureARB(GL_TEXTURE0);
    dglDisable(GL_TEXTURE_2D);
    dglAlphaFunc(GL_GREATER,0.0f);
    dglEnable(GL_ALPHA_TEST);
    dglEnable(GL_BLEND);
    dglEnable(GL_DITHER);
    dglBlendFunc(GL_DST_COLOR,0);
    dglActiveTextureARB(GL_TEXTURE0);
  }
}
...
int rGL_CheckExtension( const char *extName )
{
...
void rGL_SetMultitextureBlendMode(bool enable)
{
	if (OpenGL_tex1_enabled == enable)
		return;
	OpenGL_tex1_enabled = enable;
	if (enable) {
		dglClientActiveTextureARB(GL_TEXTURE1);
		dglActiveTextureARB(GL_TEXTURE1);
		dglEnableClientState(GL_TEXTURE_COORD_ARRAY);
