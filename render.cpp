...
void RenderFace(room *rp,int facenum)
{
	...
	if (drawn && (rp->flags & RF_FOG) && UseHardware)
		UpdateFogFace(rp, fp);
...
int Specular_face_count, Specular_visible_count;
short Specular_faces[MAX_SPECULAR_FACES];

void UpdateSpecularFace(room *room,face *face)
{
	if (Renderer_type != RENDERER_OPENGL && Specular_face_count < MAX_SPECULAR_FACES) {
		Specular_faces[Specular_face_count++] = face - room->faces;
		if (!(face->flags & FF_SPEC_INVISIBLE))
			Specular_visible_count++;
	}
}
...
void RenderRoomUnsorted(room *rp)
{
	...
	for (fn=0;fn<rp->num_faces;fn++) {
		...
		if (Render_mirror_for_room) {
			if (rp == Rooms + Room_with_mirror && fp->tmap == rp->faces[rp->mirror_face].tmap)
				continue;
		} else {
			// Clear visibility flags
			fp->flags &=~(FF_VISIBLE|FF_NOT_FACING);
		...
		fogged_portal = fp->portal_num != -1 && !(rp->portals[fp->portal_num].flags & PF_RENDER_FACES) &&
			(rp->flags & RF_FOG);
...
void ComputeRoomPulseLight(room *rp)
{
	if (!rp->pulse_time) {
		...
		float light = (rp->pulse_offset * 0.01f + Gametime) / (rp->pulse_time * 0.01f * 2);
		if (light > 1.0f)
			light = 1.0f - (light - 1.0f);
		Room_light_val = light;
	}
	if (rp->flags & RF_FLICKER) {
		ps_srand(Gametime * 1000.0f + (rp - Rooms));
		if (ps_rand() % 2)
			Room_light_val = 0.0f;
	}
}

void RoomVolumeLight(vector *pos, room *room, float *r, float *g, float *b) {
	ubyte *lights;
	if (!(lights = room->volume_lights)) {
		*r = *g = *b = 1;
		return;
	}
	float x = (pos->x - room->min_xyz.x) / 10;
	float y = (pos->y - room->min_xyz.y) / 10;
	float z = (pos->z - room->min_xyz.z) / 10;
	if (x < 0) x = 0;
	if (y < 0) y = 0;
	if (z < 0) z = 0;
	int lw = room->volume_width, lh = room->volume_height;
	int wmax = lw - 1, hmax = lh - 1, dmax = room->volume_depth - 1;
	if (x >= wmax)
		x = wmax;
	if (y >= hmax)
		y = hmax;
	if (z >= dmax)
		z = dmax;
	int lx0 = x, ly0 = y, lz0 = z;
	x -= lx0;
	y -= ly0;
	z -= lz0;
	int lx1 = min(lx0 + 1, wmax);
	int ly1 = min(ly0 + 1, hmax);
	int lz1 = min(lz0 + 1, dmax);
	ubyte l000 = lights[(lz0 * lh + ly0) * lw + lx0];
	ubyte l010 = lights[(lz0 * lh + ly1) * lw + lx0];
	ubyte l011 = lights[(lz0 * lh + ly1) * lw + lx1];
	ubyte l001 = lights[(lz0 * lh + ly0) * lw + lx1];
	ubyte l100 = lights[(lz1 * lh + ly0) * lw + lx0];
	ubyte l110 = lights[(lz1 * lh + ly1) * lw + lx0];
	ubyte l111 = lights[(lz1 * lh + ly1) * lw + lx1];
	ubyte l101 = lights[(lz1 * lh + ly0) * lw + lx1];
	float xc = 1 - x, yc = 1 - y, zc = 1 - z;
	#define LR(x) (((x) >> 5) / 7.0f)
	#define LG(x) ((((x) >> 2) & 7) / 7.0f)
	#define LB(x) (((x) & 3) / 3.0f)
	*r = z * (xc * (LR(l110) * y + LR(l100) * yc) +	x * (LR(l111) * y + LR(l101) * yc)) +
		zc * (xc * (LR(l010) * y + LR(l000) * yc) +	x * (LR(l011) * y + LR(l001) * yc));
	*g = z * (xc * (LG(l110) * y + LG(l100) * yc) +	x * (LG(l111) * y + LG(l101) * yc)) +
		zc * (xc * (LG(l010) * y + LG(l000) * yc) +	x * (LG(l011) * y + LG(l001) * yc));
	*b = z * (xc * (LB(l110) * y + LB(l100) * yc) +	x * (LB(l111) * y + LB(l101) * yc)) +
		zc * (xc * (LB(l010) * y + LB(l000) * yc) +	x * (LB(l011) * y + LB(l001) * yc));
	ComputeRoomPulseLight(room);
	*r *= Room_light_val;
	*g *= Room_light_val;
	*b *= Room_light_val;
}
...
void RenderRoom(room *rp)
{
	...
	if (Specular_face_count) {
		...
		Specular_face_count = 0;
	}

	if (Num_fog_faces_to_render) {
		RenderFogFaces(rp);
...
fog_portal_data Fog_portal_data[8];
int Num_fogged_rooms_this_frame;
void AddFogPortal(int roomnum, int portalnum)
{
	int idx = -1;
	for (int i = 0; i < Num_fogged_rooms_this_frame; i++)
		if (Fog_portal_data[i].roomnum == roomnum)
			idx = i;
	if (idx == -1) {
		if (Num_fogged_rooms_this_frame >= 8)
			return;
		Fog_portal_data[Num_fogged_rooms_this_frame].roomnum = roomnum;
		Fog_portal_data[Num_fogged_rooms_this_frame].close_dist = 10000000.0f;
		Fog_portal_data[Num_fogged_rooms_this_frame].close_face = NULL;
		idx = Num_fogged_rooms_this_frame++;
	}
	int facenum = Rooms[roomnum].portals[portalnum].portal_face;
	face *f = &Rooms[roomnum].faces[facenum];
	float dot_vert = vm_DotProduct(&f->normal, &Rooms[roomnum].verts[Rooms[roomnum].faces[facenum].face_verts[0]]);
	float dot_eye = vm_DotProduct(&f->normal, &Viewer_eye);
	float dist = dot_eye - dot_vert;
	if (Fog_portal_data[idx].close_dist > dist) {
		Fog_portal_data[idx].close_dist = dist;
		Fog_portal_data[idx].close_face = f;
	}
}
...
void CheckToRenderMineObjects(int roomnum)
{
	...
	//Add objects to sort list
	for (objnum=Rooms[roomnum].objects;(objnum!=-1) && (Num_postrenders < MAX_POSTRENDERS);objnum=Objects[objnum].next) 
	{
		ASSERT (objnum!=Objects[objnum].next);
		...
		if (obj->type == OBJ_OBSERVER && objnum == Players[obj->id].piggy_objnum)
			continue;
		...
		// Check if object is trivially rejected
		if ((obj->type==OBJ_WEAPON && Weapons[obj->id].flags & WF_ELECTRICAL) || IsPointVisible (&obj->pos,size,&zdist))
		{
			Postrender_list[Num_postrenders].type=PRT_OBJECT;
			Postrender_list[Num_postrenders].objnum = objnum;
	...
	//Add vis effects to sort list
	for (visnum=Rooms[roomnum].vis_effects;(visnum!=-1) && (Num_postrenders < MAX_POSTRENDERS);visnum=VisEffects[visnum].next) 
	{
		ASSERT (visnum!=VisEffects[visnum].next);
		...
		if (IsPointVisible (&VisEffects[visnum].pos,VisEffects[visnum].size,&zdist))
		{
			...
			Postrender_list[Num_postrenders].objnum = visnum;
			Postrender_list[Num_postrenders].z = zdist;
			Num_postrenders++;
		}
	}
...
#ifdef EDITOR
,bool render_all,bool outline,bool flat,prim *prim
#endif
)
{
#ifdef EDITOR
...
//	Flag_automap		= flag_automap;
	...
	for (int nn=N_render_rooms-1;nn>=0;nn--) 
	{
		...
		if (roomnum!=-1) 
		{
			...
			ASSERT (Rooms_visited[roomnum]!=(char)255);	
...
void RenderFogFaces (room *rp)
{
	...
	for (int faceidx = 0; faceidx < Num_fog_faces_to_render; faceidx++) {
		face *fp = &rp->faces[Fog_faces[faceidx]];
		for (int vn = 0; vn < fp->num_verts; vn++) {
			pointbuffer[vn]=*((g3Point *)&World_point_buffer[rp->wpb_index+fp->face_verts[vn]]);
			g3Point *p = &pointbuffer[vn];
  			pointlist[vn] = p;
			...
			if (Room_fog_plane_check == 0) {
				vector *v = &rp->verts[fp->face_verts[vn]];
				float dist = Room_fog_eye_distance /
					(Room_fog_eye_distance - (Room_fog_plane * *v + Room_fog_distance));
				vector pos = Viewer_eye + (*v - Viewer_eye) * dist;
				light = vm_DotProduct(&Viewer_orient.fvec, v) -
					vm_DotProduct(&Viewer_orient.fvec, &pos);
			} else if (Room_fog_plane_check == 1) {
				light = vm_DotProduct(&Room_fog_plane, &rp->verts[fp->face_verts[vn]]) +
					Room_fog_distance;
			}
			light /= rp->fog_depth;
			light = min(light, 1);
			light = max(light, 0);
			p->p3_flags |= PF_RGBA;
			p->p3_uvl.a = light * Room_light_val;
		}
...
#define MAX_FOGGED_ROOMS 8
int Room_fog_plane_check;
float Room_fog_distance, Room_fog_eye_distance;
...
void SetupRoomFog (room *rp,vector *eye,matrix *orient,int viewer_room)
{
	...
	if (viewer_room == roomnum) {
		Room_fog_plane_check = 1;
		Room_fog_distance = -vm_DotProduct(&orient->fvec, eye);
		Room_fog_plane = orient->fvec;
		return;
	}
		
	int portalidx = -1;
	for (int i = 0; i < MAX_FOGGED_ROOMS; i++)
		if (Fog_portal_data[i].roomnum == roomnum)
			portalidx = i;
	if (portalidx == -1 || !(fp = Fog_portal_data[portalidx].close_face)) {
		Room_fog_plane_check = -1;
		return;
	}
	Room_fog_plane = fp->normal;
	Room_fog_plane_check = 0;
	Room_fog_portal_vert = rp->verts[fp->face_verts[0]];
	Room_fog_distance = -vm_DotProduct(&Room_fog_plane, &Room_fog_portal_vert);
	Room_fog_eye_distance = Room_fog_plane * *eye + Room_fog_distance;
}
...
lightmap_object *RenderObjectLightmapObject;
...
void RenderObject_DrawPolymodel (object *obj,float *normalized_times)
{
	...
	if (UseHardware) {
		effect_info_s *eff = obj->effect_info;
		if (eff && (eff->type_flags & EF_FADING_OUT)) {
			pe.type = PEF_ALPHA;
			pe.alpha = eff->fade_time / eff->fade_max_time * .92f + .08f;
			use_effect = 1;
		}
		if (eff && (eff->type_flags & EF_FADING_IN)) {
			pe.type = PEF_ALPHA;
			pe.alpha = (1 - eff->fade_time / eff->fade_max_time) * .92f + .08f;
			use_effect = 1;
		}
		if (eff && (eff->type_flags & EF_CLOAKED)) {
			pe.type = PEF_ALPHA | PEF_DEFORM;
			pe.alpha = 0.13f;
			pe.deform_range = 0.1f;
			use_effect = 1;
		}
		if (obj->type == OBJ_PLAYER) {
			pe.type |= PEF_GLOW_SCALAR;
			pe.glow_length_scalar = Players[obj->id].afterburner_mag * 3 + Players[obj->id].thrust_mag;
			pe.glow_size_scalar = Players[obj->id].afterburner_mag * 0.5 + Players[obj->id].thrust_mag;
			pe.glow_length_scalar += (ps_rand() % 100 - 50) / 50 * 0.2f;
			if (Katmai)
				pe.glow_length_scalar *= 1.5;
			use_effect = 1;
		}
		// /.. add missing code
		if (use_bumpmap && (obj->type == OBJ_ROBOT || obj->type == OBJ_PLAYER)) {
			pe.bump_scalar = 1;
			pe.bump_light_pos = {obj->pos.x, obj->pos.y + 100, obj->pos.z};
			pe.type |= 0x10000;
			use_effect = true;
		}
	}
