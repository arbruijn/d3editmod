...
int ObjInitGeneric(object *objp,bool reinit)
{
	...
	if (obj_info->type == OBJ_NONE) {
		int i;
		for (i=0;i<MAX_OBJECT_IDS;i++) {		//find other object of same type
...
//		AIInit(objp, obj_info->ai_info.ai_class, obj_info->ai_info.ai_type, obj_info->ai_info.movement_type);
	...
	if(objp->control_type == CT_AI)
	{
		...
		if((objp->dynamic_wb != NULL) && !num_wbs) //((unsigned int)num_wbs != mem_size(objp->dynamic_wb)/sizeof(dynamic_wb_info)))
		{
		...
		{
			objp->dynamic_wb = (dynamic_wb_info *) mem_realloc(objp->dynamic_wb, sizeof(dynamic_wb_info) * num_wbs);
		}
...
//		WBClearInfo(objp);
	...
	if (objp->type != OBJ_POWERUP)
		objp->effect_info->type_flags=EF_VOLUME_LIT;
	if (objp->movement_type == MT_PHYSICS && objp->mtype.phys_info.rotdrag < 60.0f)
...
void ObjFree(int objnum)
{
	...
	if (objnum == Highest_object_index)
		while (Highest_object_index && Objects[--Highest_object_index].type == OBJ_NONE);
}
...
int ned_InitObjects ()
{
	...
	memset(Object_info,0,sizeof(object_info)*MAX_OBJECT_IDS);
	for(i=0;i<MAX_OBJECT_IDS;i++)
...
void ObjSetPos(object *obj,vector *pos,int roomnum,matrix *orient)
{
	...
	if (obj->roomnum != roomnum) {
		...
		// FIXME multiplayer send
		// FIXME effect_info volume
	}
}
