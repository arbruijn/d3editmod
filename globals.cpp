...
fireball Fireballs[NUM_FIREBALLS]=
	...
	{"ExplosionBlkShrk.oaf",FT_EXPLOSION,SMALL_TEXTURE,0.9f,3.0f},			// BLUE_EXPLOSION_INDEX
	{"Coolspark.ogf",FT_SPARK,SMALL_TEXTURE,1.0f,1.0f},			// GRAY_SPARK_INDEX
...
void InitDynamicLighting ()
{
	...
	for (i = 0; i < MAX_SPECULAR_INCREMENTS; i++) {
		...
		float f4 = f * f * f * f;
		float f6 = f4 * f * f;
		Specular_tables[0][i] = f6;
		Specular_tables[1][i] = f6 * f4 * f4;
		Specular_tables[2][i] = f4;
	}
}
...
int GetTextureBitmap (int handle,int framenum,bool force)
{
	...
	if (!tex->used)
		return 0;
	int bm = tex->bm_handle;
	int flags = tex->flags;
	if (flags & TF_ANIMATED) {
		PageInVClip(bm);
		int num_frames = GameVClips[bm].num_frames;
		float time = Gametime / (tex->speed / num_frames);
		int frame;
	...
	if (!(flags & TF_PROCEDURAL))
		return bm;
	if (!tex->procedural)
		AllocateProceduralForTexture(handle);
	//tex->procedural->last_procedual_frame != FrameCount
	return tex->procedural->procedural_bitmap;
#endif
}

int FindSoundName(const char *p)
{
#ifdef NEWEDITOR
...
{
#ifdef NEWEDITOR
	return ned_FindTexture(name);
#else
	int seen_used = 0;
	for (int i = 0; i < MAX_TEXTURES; i++) {
		if (seen_used >= Num_textures)
			return -1;
...
int FindTextureBitmapName(const char *name)
{
	...
	for (int i = 0; i < MAX_TEXTURES; i++)
		if (GameTextures[i].used) {
			int handle = GameTextures[i].bm_handle;
			if (GameTextures[i].flags & TF_ANIMATED) {
				PageInVClip(handle);
				for (int j = 0; j < GameVClips[handle].num_frames; j++)
					if (stricmp(GameBitmaps[GameVClips[handle].frames[j]].name, name) == 0)
						return i;
			} else if (stricmp(GameBitmaps[handle].name, name) == 0)
				return i;
		}
	return -1;
#endif
...
#define BUF_LEN 1024
static char Messagebox_title[] = "Descent 3 Level Editor";
...
bool CreateStringTable(const char *file,char * * *ptr,int *num)
{
...
int FindArg(const char *) { return 0; }


float GetFPS() { return 0.0f; }

int Debug_ErrorBox(int type, char const*title, char const*str, char const*) { return Debug_MessageBox(type, title, str); }
