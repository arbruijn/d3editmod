...
int ReadRoom(CFILE *ifile,room *rp,int version)
{
	...
	for (i=0;i<rp->num_verts;i++)
	{
		...
		// Update checksum
		Level_checksum+=(((int)rp->verts[i].x)>>4);
		Level_checksum+=(((int)rp->verts[i].y)>>4);
		Level_checksum+=(((int)rp->verts[i].z)>>4);
...
void ReadTerrainHeightChunk (CFILE *fp,int version)
{
	...
	else
	{
		...
		for (i=0;i<TERRAIN_DEPTH*TERRAIN_WIDTH;i++)
		{
			...
			Level_checksum+=Terrain_seg[i].ypos;
		}
...
int LoadLevel(const char *filename, void (*cb_fn)(const char *,int, int))
{
	CFILE *ifile;
	char tag[4];
	int n,i, retval=1;
...
// aabbs
	...
	if (version>=34 && !Dedicated_server)
	{
		...
		ASSERT (lightmap_spoken_for != NULL);
		memset (lightmap_spoken_for,0,MAX_LIGHTMAPS);

		ASSERT (free_lightmap_info != NULL);
...
int SaveLevel(const char *filename, bool f_save_room_AABB)
{
	CFILE *ofile;
	int i,highest_used_index;
	int chunk_start_pos;
...
#ifndef NEWEDITORSRC // for compatibility with Descent 3 v1.2 and earlier
