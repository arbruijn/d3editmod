...
//	Initialize DirectDraw system if we're going fullscreen, and GDIX system
	...
	hres = DX_DATA(lpDD)->SetCooperativeLevel(DX_DATA(hPrimaryWnd), 
											DDSCL_ALLOWREBOOT | 
											DDSCL_EXCLUSIVE | 
											DDSCL_FULLSCREEN);
...
bool ddgr_dx_surf_FlipVideo(ddgr_surface *sf)
{
	...
	if (bm->backbuffer && (sf->flags & SURFFLAG_BACKBUFFER)) {
		...
		if (hres == DDERR_WASSTILLDRAWING || hres == DD_OK) return true;
		else return false;
	}
...
bool ddgr_dx_surf_Blt(ddgr_surface *dsf, int dx, int dy, ddgr_surface *ssf, int sx, int sy, int sw, int sh)
{
	...
	if (ssf->flags & SURFFLAG_BACKBUFFER) lpddss = sbm->lpddsback;
	else lpddss = sbm->lpdds;

	if (dsf->flags & SURFFLAG_BACKBUFFER) lpddsd = dbm->lpddsback;
	else lpddsd = dbm->lpdds;
