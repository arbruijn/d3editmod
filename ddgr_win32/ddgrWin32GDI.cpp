	...
	//	if we are in fullscreen, do display mode change
		...
		if (GDI_DATA(fullscreen)) {
			...
			// grab all device modes for the current display device.
				...
				{
					LONG lres;
					if ((int)GDI_DATA(devmodes[i].dmBitsPerPel)==sf->bpp &&
							(int)GDI_DATA(devmodes[i].dmPelsWidth)==sf->w &&
...
//	Are we clearing the primary display?
	if (dbm->hbm == NULL) {
		if (dbm->hwnd) hdc = GetDC(dbm->hwnd);
		else hdc = GetDC(GDI_DATA(hPrimaryWnd));
	}
	else {
	...
	// do if clearing was done on primary display.
		if (dbm->hwnd) ReleaseDC(dbm->hwnd, hdc);
		else ReleaseDC(GDI_DATA(hPrimaryWnd), hdc);
	}
	else {
...
//	else we use the DC of the window specified in the destination.
	if (dbm->hbm == NULL) {
		if (dbm->hwnd == NULL) hdc_dest = GetDC(GDI_DATA(hPrimaryWnd));
		else hdc_dest = GetDC(dbm->hwnd);
	}
	else {
	...
	if (dbm->hbm == NULL) {
		if (dbm->hwnd == NULL) ReleaseDC(GDI_DATA(hPrimaryWnd), hdc_dest);
		else ReleaseDC(dbm->hwnd, hdc_dest);
	}
	else {
