...
class grPen;
#else
typedef struct grMemorySurface grMemorySurface;
#endif
...
typedef enum grTextAlign 
{					// used to align text along viewport
	grTextUnjustified,
	grTextLeft,
	grTextCentered,
	grTextRight,
...
public:	// Accessor functions
	char *get_name() const { if (m_FontHandle == -1) return NULL; else return m_FontList[m_FontHandle].name; };
	int min_ascii() const { ASSERT(m_FontHandle > -1); return m_FontList[m_FontHandle].font.min_ascii; };
	int max_ascii() const { ASSERT(m_FontHandle > -1); return m_FontList[m_FontHandle].font.max_ascii; };
	int height() const { ASSERT(m_FontHandle > -1); return m_FontList[m_FontHandle].font.height; };
...
public:
	...
	int width() const { ASSERT(m_SurfInit); return ddsfObj.w; };
	int height() const { ASSERT(m_SurfInit); return ddsfObj.h; };
	int bpp() const { ASSERT(m_SurfInit); return ddsfObj.bpp; };
	unsigned flags() const { ASSERT(m_SurfInit); return ddsfObj.flags; };

	int rowsize() const { ASSERT(m_SurfInit); return m_DataRowsize; };
	char *data() const { ASSERT(m_SurfInit); return m_DataPtr; };
...
protected:
	...
	int vp_InitLeft, vp_InitTop, 
		vp_InitRight, vp_InitBottom;		// viewport's initial global screen coords
...
inline ddgr_color grViewport::get_text_color() const 
{
	return char_Props.col[0];
}
