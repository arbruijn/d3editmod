...
extern void RoomVolumeLight(vector *pos, room *room, float *r, float *g, float *b);
extern vector RenderObject_LightDirection;

extern void RenderObject_SetStatic(float r,float g,float b);
extern void RenderObject_SetGouraud(vector *dir,float r,float g,float b,float scalar);
extern void RenderObject_SetLightmaps(lightmap_object *lmobject);
extern void RenderObject_SetLightDirection(vector *dir);

bool SetupMineObject(object *obj) {
	ubyte light_type = obj->lighting_render_type;
	if (light_type && Poly_models[obj->rtype.pobj_info.model_num].new_style) {
		if (light_type == LRT_GOURAUD || NoLightmaps) {
			effect_info_s *eff = obj->effect_info;
			float r = 1, g = 1, b = 1;
			vector dir = {0, -1, 0};
			if (eff && (eff->type_flags & EF_VOLUME_LIT) && !(Rooms[obj->roomnum].flags & RF_EXTERNAL)) {
				vector pos = obj->pos;
				if (Render_mirror_for_room)
					pos = obj->last_pos;
				if (eff->type_flags & EF_VOLUME_CHANGING) {
					float o_r, o_g, o_b, n_r, n_g, n_b;
					RoomVolumeLight(&eff->volume_old_pos, Rooms + eff->volume_old_room, &o_r, &o_g, &o_b);
					RoomVolumeLight(&pos, Rooms + obj->roomnum, &n_r, &n_g, &n_b);
					r = o_r * eff->volume_change_time + n_r * (1 - eff->volume_change_time);
					g = o_g * eff->volume_change_time + n_g * (1 - eff->volume_change_time);
					b = o_b * eff->volume_change_time + n_b * (1 - eff->volume_change_time);
				} else
					RoomVolumeLight(&pos, Rooms + obj->roomnum, &r, &g, &b);
				r += eff->dynamic_red;
				if (r > 1) r = 1;
				g += eff->dynamic_green;
				if (g > 1) g = 1;
			...
			RenderObject_SetGouraud(&dir, r, g, b, 1);
		} else if (light_type == LRT_LIGHTMAPS) {
...
bool SetupTerrainObject(object *obj) {
	float r, g, b, static_light;
	vector dir = Terrain_sky.lightsource;
	vm_NormalizeVector(&dir);
	obj->flags |= OF_SAFE_TO_RENDER;
	RenderObject_SetLightDirection(&dir);
	rend_SetColorModel(CM_MONO);
	if (obj->render_type != RT_POLYOBJ &&
		(obj->render_type != RT_WEAPON || (Weapons[obj->id].flags & (WF_IMAGE_BITMAP | WF_IMAGE_VCLIP)))) {
		RenderObject_SetStatic(1.0,1.0,1.0);
		return true;
	}
	if (obj->type == OBJ_POWERUP) {
		...
		effect_info_s *eff = obj->effect_info;
		static_light = GetTerrainDynamicScalar(&obj->pos,obj->roomnum & 0x7fffffff);
		r = g = b = static_light;
		if (eff && (eff->type_flags & EF_VOLUME_LIT)) {
			float r = eff->dynamic_red + static_light, g = eff->dynamic_green + static_light, b = eff->dynamic_blue + static_light;
			if (r > 1) r = 1;
			if (g > 1) g = 1;
			if (b > 1) b = 1;
			if (obj->type == OBJ_ROBOT) {
	...
	ubyte light_type = obj->lighting_render_type;
	if (light_type == LRT_STATIC || !Poly_models[obj->rtype.pobj_info.model_num].new_style) {
		RenderObject_SetStatic(r, g, b);
	} else {
		if (light_type == LRT_GOURAUD || NoLightmaps) {
			vector dir = {0, -1, 0};
			RenderObject_SetGouraud(&dir, r, g, b, static_light);
		} else if (light_type == LRT_LIGHTMAPS) {
			if (obj->lm_object.used)
				RenderObject_SetLightmaps(&obj->lm_object);
			else
