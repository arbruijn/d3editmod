...
bool BOA_PassablePortal(int room, int portal_index, bool f_for_sound, bool f_making_robot_path_invalid_list)
{
	...
	if(!BOA_f_making_boa)
	{
		...
		if((Rooms[room].portals[portal_index].flags & PF_RENDER_FACES) && !(Rooms[room].portals[portal_index].flags & PF_RENDERED_FLYTHROUGH) || (Rooms[room].portals[portal_index].flags & PF_BLOCK))
		{
			return false;
		}
	}
	else
	{
		...
		if((Rooms[room].portals[portal_index].flags & PF_BLOCK) && !(Rooms[room].portals[portal_index].flags & PF_BLOCK_REMOVABLE))
			return false;

		if((Rooms[room].portals[portal_index].flags & PF_RENDER_FACES) && !(Rooms[room].portals[portal_index].flags & PF_RENDERED_FLYTHROUGH))
		{
...
int BOA_DetermineStartRoomPortal(int start_room, vector *start_pos, int end_room, vector *end_pos, bool f_for_sound, bool f_making_robot_path_invalid_list, int *blocked_portal)
{
	int i;
...
bool BOA_ComputeMinDist(int start_room, int end_room, float max_check_dist, float *dist, int *num_blockages)
{
	*dist = 0.0f;
	...
	do
	{
		...
		if(cur_room <= Highest_room_index && num_blockages && (Rooms[cur_room].flags & RF_DOOR) && (cur_room != end_room))
		{
...
//				if(region != 0)
//				{
//					BOA_connect[0][BOA_num_connect[region]].roomnum = Rooms[i].portals[j].croom;
//					BOA_connect[0][BOA_num_connect[region]].portal = Rooms[i].portals[j].cportal;
//				}
...
//	mprintf((0, "Find path for %d to %d\n", i, j));
	...
	while(cur_node = PQPath.pop()) 
	{
		...
		ASSERT(BOA_INDEX(cur_node->roomnum) >= 0 && BOA_INDEX(cur_node->roomnum) <= Highest_room_index + MAX_BOA_TERRAIN_REGIONS);
		...
		if(i > j) 
		{
			...
			for(counter = 0; counter < num_portals; counter++) 
			{
				...
				new_cost = cur_node->cost + BOA_cost_array[BOA_INDEX(cur_node->roomnum)][counter] + BOA_cost_array[BOA_INDEX(next_room)][next_portal];
		...
		{	
			...
			for(counter = 0; counter < num_portals; counter++) 
			{
				...
				{
					ASSERT(cur_node->roomnum <= Highest_room_index);
					next_room = Highest_room_index + TERRAIN_REGION(GetTerrainCellFromPos(&Rooms[cur_node->roomnum].portals[counter].path_pnt)) + 1;
				}
...
{	
	...
	for(i = 0; i <= Highest_room_index + MAX_BOA_TERRAIN_REGIONS; i++) 
	{
		...
		for(j = Highest_room_index + MAX_BOA_TERRAIN_REGIONS; j >= 0; j--) 
		{
			...
			if(i != j && BOA_Array[i][j] == i) FindPath(i, j);
			if(i != j && BOA_Array[j][i] == j) FindPath(j, i);
		}
	}
}
...
void ValidateRoomPathPoint(int room, char *message, int len)
{
	...
	if(IsPathPointValid(room, &pos)) return;
		...
		while (stack_count > 0)
		{
			...
			for (t = 0; t < rp->num_portals && !done; t++)
			{
				...
				for (j=0;j<Rooms[check_room].num_portals && !done;j++)
				{
					...
					ComputeBOAVisFaceUpperLeft (&Rooms[check_room],dest_fp,&dest_upper_left,&dest_width,&dest_height,&dest_center);
						...
						for (int sx=0;sx<src_width && !done;sx++,src_vector+=src_matrix.rvec)
						{
							...
							for (int dy=0;dy<dest_height && !done;dy++,dest_ybase-=dest_matrix.uvec)
							{
								...
								for (int dx=0;dx<dest_width && !done;dx++,dest_vector+=dest_matrix.rvec)
								{
									...
									if(!(Rooms[check_room].flags & RF_EXTERNAL) && (!fvi_QuickRoomCheck(&dest_vector, &Rooms[check_room], false) && !fvi_QuickRoomCheck(&dest_vector, &Rooms[check_room], true)))
										continue;
									...
									fq.thisobjnum			= -1;
									fq.ignore_obj_list	= NULL;
									fq.flags					= FQ_BACKFACE | FQ_IGNORE_RENDER_THROUGH_PORTALS;  // Gaurentees in wierd geometry cases (as the ray will hit as it comes back in the mine)
									                                     // that the ray cannot start outside the mine like with a slightly non-planar 
									                                     // portal or other non-nice situations like zero-width door portal/room-face combos
...
void ComputeAABB(bool f_full)
{
	...
	//			mprintf((0, "Room %d Best Shell is %d\n", i, best));
		...
		for(i = 0; i <= Highest_room_index; i++)
		{
			if(Rooms[i].used)
			{
				...
				if(min_xyz.x <= rp->min_xyz.x) min_xyz.x = rp->min_xyz.x + 2.5f;
				if(min_xyz.y <= rp->min_xyz.y) min_xyz.y = rp->min_xyz.y + 2.5f;
				if(min_xyz.z <= rp->min_xyz.z) min_xyz.z = rp->min_xyz.z + 2.5f;
				if(max_xyz.x >= rp->max_xyz.x) max_xyz.x = rp->max_xyz.x - 2.5f;
				if(max_xyz.y >= rp->max_xyz.y) max_xyz.y = rp->max_xyz.y - 2.5f;
				if(max_xyz.z >= rp->max_xyz.z) max_xyz.z = rp->max_xyz.z - 2.5f;
	...
	//						mprintf((0, "%d %d %d\n", num_faces[sp][0], num_faces[sp][1], num_faces[sp][2]));
							...
							if(fabs(num_faces[sp][0] - num_faces[sp][1]) > fabs(num_faces[sp][1] - num_faces[sp][2]))
								diff[sp] = fabs(num_faces[sp][0] - num_faces[sp][1]);
							else
								diff[sp] = fabs(num_faces[sp][1] - num_faces[sp][2]);
						}
	...
	//					mprintf((0, "Split Plane is %d\n", best));
						...
						if(rp->num_bbf_regions <= MAX_REGIONS_PER_ROOM - 2)
						{
							...
							for(count = 0; count < rp->num_bbf[j]; count++)
							{
								...
								{
									rp->bbf_list[rp->num_bbf_regions + 1][rp->num_bbf[rp->num_bbf_regions + 1]++] = rp->bbf_list[j][count];
	...
	//				mprintf((0, "Region %d has %d faces in it.\n", i, rp->num_bbf[i]));
		...
		for(count = 0; count <= Highest_room_index; count++)
		{
			if(Rooms[count].used)
			{
				...
				rp->bbf_list_sector = (unsigned char *) mem_realloc(rp->bbf_list_sector, rp->num_bbf_regions * sizeof(unsigned char));
			}
		}
