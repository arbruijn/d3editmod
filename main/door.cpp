...
void InitializeDoorData(int slot)
{
...
void MarkDoorInUse(int slot,bool inuse)
{
		...
		if(Doors[slot].ref_count==0)
		{
			...
			if(Doors[slot].model_handle>=0)
			{
				...
				for (int i=0; i<pm->n_textures; i++)
					MarkTextureInUse(pm->textures[i],true);
			}
		}
...
int AllocDoor(char *name,int tablefile)
{
	if(name)
	{
		...
		int old_id = FindDoor(name);
		if(old_id!=-1)
		{
			//this item is already in memory!
			FreeDoorData(old_id);
			InitializeDoorData(old_id);			
	...
	for(i=0;i<MAX_DOORS;i++)
	{
		...
		{
			index = i;
			memset(&Doors[i],0,sizeof(door_info));
...
void FreeDoor(int slot,bool force_unload)
{
	ASSERT(slot>=0 && slot<MAX_DOORS);
	if(slot<0 || slot>=MAX_DOORS)
		return;
