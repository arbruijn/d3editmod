...
char *MatcenEffectStrings[NUM_MATCEN_EFFECTS] =
{
"Line lightning",
"Line sinewave",
...
bool matcen::DoObjProd()
{
	...
	if(m_prod_type[m_cached_prod_index] >= 0)
	{
		...
		objnum = ObjCreate(Object_info[m_prod_type[m_cached_prod_index]].type, m_prod_type[m_cached_prod_index], croom, &m_create_pnt, &orient, p_handle);
		Matcen_created = false;

		if(objnum >= 0 && (Object_info[m_prod_type[m_cached_prod_index]].flags & OIF_CONTROL_AI) && (Object_info[m_prod_type[m_cached_prod_index]].ai_info->movement_type != MC_FLYING))
		{
			vector gp;
			...
			if(PhysCalcGround(&gp, NULL, obj, 0))
			{
				...
				if(fate != HIT_NONE)
				{
					...
					if(ps != pr)
					{
						...
						ObjSetPos(obj, &obj->pos, obj->roomnum, NULL);
...
int matcen::GetAttach()
{
	return m_roomnum;
}

char matcen::GetAttachType()
{
	return m_type;
}
...
char matcen::GetNumSpawnPnts()
{
	return m_num_spawn_pnts;
}
...
bool matcen::ComputeCreatePnt()
{
	...
	else if(m_type != MT_UNASSIGNED)
	{
		...
		if(m_cached_prod_index >= 0 && m_cached_prod_index < m_num_prod_types && m_prod_type[m_cached_prod_index] >= 0 && m_prod_type[m_cached_prod_index] <= MAX_OBJECT_TYPES && Object_info[m_prod_type[m_cached_prod_index]].type != OBJ_NONE)
		{
...
char matcen::GetNumProdTypes()
{
	return m_num_prod_types;
}
...
float matcen::GetProdMultiplier()
{
	return m_speed_multi;
}
...
{
	m_next_active_check_time = Gametime + MATCEN_ACTIVE_CHECK_RATE + (MATCEN_ACTIVE_CHECK_VARIENCE * (((float)ps_rand())/((float)RAND_MAX) - .5f));
	m_last_active_check_result = (m_status & MSTAT_ACTIVE) != 0;
	...
	else
	{
		switch(m_control_type)
		{
			...
			case MPC_AFTER_PLAYER_NEAR:
			{
				...
				else if(ROOMNUM_OUTSIDE(m_create_room) || (m_create_room <= Highest_room_index && Rooms[m_create_room].used && (Rooms[m_create_room].flags & RF_EXTERNAL)))
				{
...
void matcen::DoThinkFrame()
{
	...
	if(m_type == MT_OBJECT)
	{
		...
		if((m_prod_mode != MMODE_NOTPROD && (parent->movement_type == MT_PHYSICS || parent->movement_type == MT_WALKING)) || 
	...
	if(!(m_status & MSTAT_NOT_HURT_PLAYER))
	{
		if(m_prod_mode == MMODE_PREPROD || m_prod_mode == MMODE_POSTPROD)
		{
...
void matcen::SetCreationTexture(short texnum)
{
	m_creation_texture=texnum;
}

short matcen::GetCreationTexture()
{
	return m_creation_texture;
}
...
void matcen::Reset()
{
	...
	// Resets the a matcen
	m_status &= ~(MSTAT_DONE_PROD | MSTAT_ACTIVE | MSTAT_ACTIVE_PAUSE | MSTAT_PROD_TILL_DONE | MSTAT_PROD_ONE_PAUSE | MSTAT_PROD_ONE_DISABLE);
...
int matcen::GetMaxAliveChildren()
{
	return m_max_alive_children;
}
...
float matcen::GetPreProdTime()
{
	return m_preprod_time;
}
...
float matcen::GetPostProdTime()
{
	return m_postprod_time;
}
...
{
	return (id >= 0 && id < Num_matcens && Matcen[id]);
}
