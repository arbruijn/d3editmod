...
small_point *World_point_buffer;	// Rotated points
...
void SetupSky (float radius,int flags,ubyte randit)
{
	...
	for (i=0;i<6;i++)
	{
		...
		{
			matrix tempm;
			int increment=16384/5;
			vector *vec=&Terrain_sky.horizon_vectors[t][i];

			angle pitch;

			pitch=(65536-16384)+(i*increment);
...
void InitTerrain (void)
{
	...
	if (!Dedicated_server)
	{
		...
		World_point_buffer=(small_point *)mem_malloc (TERRAIN_WIDTH*TERRAIN_DEPTH*sizeof(small_point));
